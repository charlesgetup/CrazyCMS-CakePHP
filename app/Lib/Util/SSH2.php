...
class SSH2 {
	...
	public function __construct ( $host, $user, $pass, $port, $debug = false, $pubk = null ) {
		$this->host = $host;
		$this->user = $user;
		$this->pass = $pass;
		$this->port = $port;
	...
	public function authenticate () {
		...
		if(!empty($this->pubk)){
			if(file_exists($this->pass) && file_exists($this->pubk)){
				if(ssh2_auth_pubkey_file($this->conn, $this->user, $this->pubk, $this->pass)){
					return $this;
				}
			}
		}else{
