...
App::uses('HtmlDomParser', 'Vendor/PhpSimpleHTMLDom');
...
class AutoDescription {
	...
	public function getSummary($params, $encoding, $removedHTMLTags = array()){

		$this->encoding = $encoding;
		mb_internal_encoding($encoding);
		$this->contents = $this->replaceChars($params['content'], $removedHTMLTags);

		$summary = $this->summarizer->get_summary($this->contents);

		if($this->debug){
			error_log(print_r($this->summarizer->how_we_did(), true));
		}

		$summary = str_ireplace($params['companyName'], $params['companyName'], $summary); // If the summary contains company name, make sure the case matches the company's special format.

		return $summary;
	}

/**
 * Convert HTML to article, remove non-sense text
 * @param string $content
 * @return string
 */
	private function replaceChars($content, $removedHTMLTags){
		...
		// Remove some HTML tags which may not contains sentences and paragraphs
		$extendPHP = new ExtendPHP();
		$content = $extendPHP->stripTagsWithContent($content, array('ul', 'ol', 'a', 'table'));

		// Remove certain tags by CSS selectors
		if(!empty($removedHTMLTags) && is_array($removedHTMLTags)){
			$dom = HtmlDomParser::str_get_html( $content );
			foreach($removedHTMLTags as $cssSelector){
				foreach($dom->find($cssSelector) as &$tag){
					$tag->outertext = '';
				}
				$dom->load($dom->save());
			}
...
class Summarizer{
	...
	public function split_content_to_sentences($content) {
		...
		foreach($content as $key => $value) {
			...
			$wordArr = explode(" ", $value); // Discard sentence which contains less than 5 words
			if(count($wordArr) > 5){
	...
	public function get_summary($content) {
		...
		// Final format check and clean up
		$this->summary = trim($this->summary);
		$this->summary = ucfirst($this->summary);
		if(substr($this->summary, -1) != "."){
			$this->summary .= ".";
		}
