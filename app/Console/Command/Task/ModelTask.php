 ...
 */
	protected function _getModelObject($className, $table = null) {
        ...
        if(!in_array($table, $this->getAllTables($this->connection)) && isset($this->params["plugin"]) && !empty($this->params["plugin"])){
            $pluginTable = Inflector::underscore($this->params["plugin"]) ."_" .$table;
            if(in_array($pluginTable, $this->getAllTables($this->connection))){
            	$table = $pluginTable;
            }
        }
