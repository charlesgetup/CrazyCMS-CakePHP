...
class DataTableComponent extends Component{
	...
	public function getResponse($controller = null, $model=null){
		...
		// Because we add "Log" variable in app controller class, if the actual controller is LogsController and it will probably got the empty AppController var.
		if(empty($this->model) && !empty($this->controller)){
			$modelName = $this->controller->modelClass;
			if($modelName == "Log"){
		...
		if(isset($this->controller->request->data)){
			$httpGet = $this->controller->request->data;
		}
		...
		// set sql limits
		if( isset($this->controller->request->data['iDisplayStart']) && $this->controller->request->data['iDisplayLength'] != '-1' ){
			$start = $this->controller->request->data['iDisplayStart'];
			$length = $this->controller->request->data['iDisplayLength'];
		...
		$response = array(
				'sEcho' => isset($this->controller->request->data['sEcho']) ? intval($this->controller->request->data['sEcho']) : 1,
				'iTotalRecords' => $total,
		...
		array_walk_recursive($response['aaData'], function (&$v, $k){
			$k = strtolower($k);
			if($k == "active"){
				$v = empty($v) ? '<span class="red">' .__("Inactive") .'</span>' : '<span class="green">' .__("Active") .'</span>';
			}
		});
	...
	private function getOrderByStatements(){
		...
		if(empty($this->controller->request->data['iSortCol_0'])){
			if(substr($this->controller->request->data['mDataProp_' .$this->controller->request->data['iSortCol_0']], -10) == ".selection"){
				$columnIndex = 0;
				$this->controller->request->data['iSortCol_0'] = 0; // Set it to zero and it will be used as next step condition
				while(isset($this->controller->request->data['bSortable_' .$columnIndex])){
					if($this->controller->request->data['bSortable_' .$columnIndex] === true || strtolower($this->controller->request->data['bSortable_' .$columnIndex]) === 'true'){
						$this->controller->request->data['iSortCol_0'] = $columnIndex;
						break;
					}
					$columnIndex++;
				}
			...
			if($this->controller->request->data['bSortable_' .$this->controller->request->data['iSortCol_0']] !== true && strtolower($this->controller->request->data['bSortable_' .$this->controller->request->data['iSortCol_0']]) !== 'true'){
				$this->controller->request->data['iSortCol_0'] = 0;
			}
		}

		// If we cannot find a suitable column to sort, then don't sort.
		if($this->controller->request->data['iSortCol_0'] == 0){
			unset($this->controller->request->data['iSortCol_0']);
		}

		if( $this->mDataProp == true ){
			$direction = $this->controller->request->data['sSortDir_0'] === 'asc' ? 'asc' : 'desc';
			$mDataProp = $this->controller->request->data['iSortCol_0'];
			$orderBy = $this->controller->request->data['mDataProp_'.$mDataProp].' '.$direction.', ';
		}else if( !empty($fields) && isset($this->controller->request->data['iSortCol_0']) ){
			$direction = $this->controller->request->data['sSortDir_0'] === 'asc' ? 'asc' : 'desc';
			$orderBy = $fields[ $this->controller->request->data['iSortCol_0'] ].' '.$direction.', ';
		}

		if(!empty($orderBy)){
			$orderBy = substr($orderBy,0, -2);

			// Only return order clause when we actually find a sortable column.
			$orderByTrimStr = trim($orderBy);
			if(in_array($orderByTrimStr, array('asc', 'desc'))){
				$orderBy = '';
			}else{
				return $orderBy;
			}
	...
	private function getWhereConditions(){
		...
		if($this->mDataProp == true){
			for($i=0;$i<$this->controller->request->data['iColumns'];$i++){
				if(!isset($this->controller->request->data['bSearchable_'.$i]) || $this->controller->request->data['bSearchable_'.$i] == true){
					$fields[] = $this->controller->request->data['mDataProp_'.$i];
				}
			}
		}
		...
		foreach($fields as $x => $column){
			...
			if( $this->controller->request->data['bSearchable_'.$x] == 'true' ){

				if($this->mDataProp == true){
					$conditions['OR'][] = array(
							$this->controller->request->data['mDataProp_'.$x].' LIKE' => '%'.$this->controller->request->data['sSearch'].'%'
					);
				}
				else{
					...
					if( $this->conditionsByValidate == 1 && array_key_exists($column,$this->model->validate) ){
						...
						if( !empty($this->controller->paginate['contain']) ){
							if(array_key_exists($table, $this->controller->paginate['contain']) && in_array($field, $this->controller->paginate['contain'][$table]['fields'])){
								$conditions[$table]['conditions'][] = $column.' LIKE "%'.$this->controller->request->data['sSearch'].'%"';
							}
						}
						else{
							$conditions['OR'][] = array(
									$column.' LIKE' => '%'.$this->controller->request->data['sSearch'].'%'
							);
						}
					}
				}
	...
	private function conditionByDataType($field){
		foreach($this->model->validate[$field] as $rule => $j){
			switch($rule){
				...
				case 'numeric':
				case 'naturalNumber':
					$condition = array($field => $this->controller->request->data['sSearch']);
					break;
			}
		}
		return $condition;
