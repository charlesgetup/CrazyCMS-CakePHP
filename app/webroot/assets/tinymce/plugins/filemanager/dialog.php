...
if(isset($_GET['subfolder']) && !empty($_GET['subfolder'])) {
    ...
    $thumbs_path = $root .$upload_root_dir .'/thumbs/';
    if(stristr($thumbs_path, $subfolder . '/') === FALSE){
    	$thumbs_path .= $subfolder . '/';
    }
    if(strstr($thumbs_path, "//")){ $thumbs_path = str_replace("//", "/", $thumbs_path); }

    $listFiles = $AmazonS3->listBucket('?prefix=' .$thumbs_path.$subdir .'&delimiter=/');
    if (empty($listFiles)){
...
if(isset($cur_dir) && !empty($cur_dir)){
	if(strstr($cur_dir, "//")){ $cur_dir = str_replace("//", "/", $cur_dir); }
}
...
<div class="uploader">
	<?php
		$selfUrl = Router::url(null, true) .$_SERVER['PHP_SELF'];
		$selfUrl = str_ireplace("//", "/", $selfUrl);
	?>
	<form action="<?php echo $selfUrl; ?>" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
...
<!----- header div end ------->
	...
	<div class="span12">
		<?php
			$files = $AmazonS3->listBucket('?prefix=' .$root . $cur_dir .'&delimiter=/', true);
			if(empty($files)){
				create_folder($root . $cur_dir);
				$files = $AmazonS3->listBucket('?prefix=' .$root . $cur_dir .'&delimiter=/', true);
			}
		?>
	    <?php if(empty($files)){ ?>
	    <br/>
	    <div class="alert alert-error"><?php echo __('Error! The root folder not exist.'); ?></div>
	    <?php }else{ ?>
	    <h4 id="help"><?php echo __('Swipe the name of file/folder to show options'); ?></h4>
		...
		foreach ($files as $file) {
			...
			$is_dir = substr($file, -1) == "/" || ($pathInfo['filename'] != "empty" && !isset($pathInfo['extension']));

		    if ($is_dir) {
			//add in thumbs folder if not exist
			list($url_status) = get_headers($thumbs_path.$subdir.$pathInfo['basename']);
			if (strpos($url_status, "404") !== FALSE) create_folder(false,$thumbs_path.$subdir.$pathInfo['basename']);
			$class_ext = 3;
			$src = $subdir . $pathInfo['basename']."/";
			...
			<li>
				<figure>
					...
					<img class="directory-img"  src="ico/folder<?php if($pathInfo['basename']=='..') echo "_return"?>.png" alt="folder" />
					</div>
					</div>
					</a>
				    <div class="box">
					<h4><?php echo $pathInfo['basename'] ?></h4>
				    </div>
				    <figcaption>
					    <a href="javascript:void('')"
						class="erase-button"
						<?php if($delete_folder){ ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo $root. $cur_dir .$pathInfo['basename']; ?>','<?php echo $thumbs_path.$subdir .$pathInfo['basename']; ?>'); $(this).parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>">
			...
			if(!is_numeric($nu)){
				$nu = md5(strval($nu) .time() .rand(0, 999));
			}
			...
			if (!$is_dir) {
				...
				$src = $base_url . $cur_dir . $pathInfo['basename'];
				$src_thumb = $thumbs_path.$subdir. $pathInfo['basename'];

				$is_img=true;
				//check if is smaller than thumb
				$info=getimagesize($file);
				if($info[0]<122 && $info[2]<91){
				    $src_thumb = $current_path_root.DIRECTORY_SEPARATOR.$subfolder.DIRECTORY_SEPARATOR.$subdir.$pathInfo['basename'];
				    $show_original=true;
				}
			    ...
			    if((!($_GET['type']==1 && !$is_img) && !($_GET['type']>=3 && !$is_video))){
					...
					// Update file path, use URL path or relative path, not absolute path
					if(substr($src_thumb, 0, 1) == "/"){
						if(substr($current_path_root, 0, 1) == "/"){

						}else{
							$current_path_pieces = explode("/", $current_path_root);
							foreach($current_path_pieces as &$piece){
								if($piece == ".."){
									$piece = null;
								}
							}
							$current_path_pieces = array_filter($current_path_pieces);
							$path_delimiter = implode("/", $current_path_pieces);
							$src_thumb_pieces = explode($path_delimiter, $src_thumb);
							$src_thumb = $current_path_root .DIRECTORY_SEPARATOR .array_pop($src_thumb_pieces);
						}
					}
...
?>
				...
				<figure>
					<a href="javascript:void('');" title="<?php echo  lang_Select?>" onclick="<?php echo $apply."('".$pathInfo['basename']."',".$_GET['type'].",'".$_GET['field_id']."');"; ?>">
					<div class="img-precontainer">
					<div class="img-container"><span></span>
					<img data-src="holder.js/122x91" alt="image" <?php echo $show_original ? "class='original'" : "" ?> src="<?php echo $base_url .$src_thumb; ?>">
					</div>
					</div>
					</a>
					<div class="box">
					<h4><?php echo substr($pathInfo['basename'], 0, '-' . (strlen($file_ext) + 1)); ?></h4>

					</div>
					<figcaption>
					    <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
						<input type="hidden" name="path" value="<?php echo $root. $cur_dir. $pathInfo['basename']?>"/>
						<input type="hidden" name="name" value="<?php echo $pathInfo['basename']?>"/>
					    ...
					    <?php if($delete_file){ ?>onclick=" if(confirm('<?php echo lang_Confirm_del; ?>')){ delete_file('<?php echo $root. $cur_dir .$pathInfo['basename']; ?>','<?php echo $thumbs_path.$subdir .$pathInfo['basename']; ?>'); $(this).parent().parent().parent().parent().hide(200); return false;}"<?php } ?> title="<?php echo lang_Erase?>"><i class="icon-trash <?php if(!$delete_file) echo 'icon-white'; ?>"></i></a>
					    </form>
					</figcaption>
				</figure>
