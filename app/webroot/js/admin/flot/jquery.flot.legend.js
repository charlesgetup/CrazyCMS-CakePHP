...
(function($) {
    ...
    function insertLegend(plot, options, placeholder, legendEntries) {
        ...
        var fill = options.legend.backgroundColor ? 'fill="' + options.legend.backgroundColor +'" ' : '';
        var opacity = '';
        if (options.legend.backgroundOpacity) {
        	//opacity = 'stroke-opacity="' + options.legend.backgroundOpacity + '" '; // This transparent the whole thing, not only the background
        	fill = 'fill="none" '; // Remove background color
        }
        html[j++] = '<svg class="legendLayer" ' + fill + opacity + 'stroke-width="1" strock-linecap="round" style="width:inherit;height:inherit;">';
        ...
        for (var i = 0; i < entries.length; ++i) {
            ...
            labelHtml = '<text x="' + shape.xPos + '" y="' + shape.yPos + '" text-anchor="start" font-size="100%" ' + (options.legend.strokeColor ? 'fill="' + options.legend.strokeColor + '"' : '') + '><tspan dx="2em" dy="1.2em">' + shape.label + '</tspan></text>'
