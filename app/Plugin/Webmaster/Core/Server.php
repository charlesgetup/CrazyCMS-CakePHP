 ...
 */
	public static function convert($size, $useThisUnit = false) {
		if (!$size) {
			return '0 b';
		}
		...
		if($useThisUnit && in_array($useThisUnit, $unit)){

			$specifiedUnitIndex = array_search($useThisUnit, $unit);
			if($specifiedUnitIndex > $i){

				for($j = 0; $j < abs($specifiedUnitIndex - $i); $j++){
					$size /= 1024;
				}

			}elseif ($specifiedUnitIndex < $i){
 ...
 */
	public static function info() {
		...
		if (!$load) {
			...
			if ($load) {
				$load = explode(' ', $load);
				$load[2] = trim(array_pop($load)); // load averages for 15 mins
				$load[1] = str_replace(',', '', array_pop($load)); // load averages for 5 mins
				$load[0] = str_replace(',', '', array_pop($load)); // load averages for 1 min

				//TODO show this in backend
				$a = @shell_exec('uptime -s'); // Get date time since when system has been running
