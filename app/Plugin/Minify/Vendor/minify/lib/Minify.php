...
class Minify
{
    ...
    public function errorExit($header, $url = '', $msgHtml = '')
    {
        ...
        if(in_array($_SERVER, $localhostArr)){
        	if ($msgHtml) {
        		echo $msgHtml;
        	}
        	if ($url) {
    ...
    /**
     * Check whether the given file needs to be obfuscated
     * @param object $source
     * @param string $content
     * @param string $type
     * @return string
     */
    protected function obfuscateSourceFile($sourceFilePath, $content, $type){

    	if(!empty($content) && !empty($sourceFilePath)){
    		$sourceFileName = basename($sourceFilePath);
			$canEncrypt = false;
			foreach($this->options['encryptFilesIdentifier'] as $needle){
				if($canEncrypt){
					break;
				}else{
					$canEncrypt = !empty($needle) && substr(strtolower($sourceFileName), -strlen(strtolower($needle))) === $needle;
				}
			}
    		if($canEncrypt){
    			if ($this->options['encryptProcessorRequire']) {
    				require_once $this->options['encryptProcessorRequire'];
    			}
    			if ($this->options['encryptProcessor']) {
    				$content = call_user_func($this->options['encryptProcessor'], $content, $type, $sourceFileName);
    			}
    		}
    	}
    ...
    protected function combineMinify()
    {
        ...
        do {
            ...
            if ($i > 0                               // yes, we have at least the first group populated
                ...
                if ($lastMinifier) {
                    try {
                    	if(!empty($groupToProcessTogether)){
                    		foreach($groupToProcessTogether as $sourceFilePath => &$obfuscatedContent){
                    			$originalContent = $obfuscatedContent;
                    			$obfuscatedContent = $this->obfuscateSourceFile($sourceFilePath, $obfuscatedContent, $type); // Obfuscate first
                    			if($obfuscatedContent == $originalContent){
                    				$obfuscatedContent = call_user_func($lastMinifier, $obfuscatedContent, $lastOptions);
                    			}
                    		}
                    	}
                    } catch (Exception $e) {
            ...
            if ($source) {
            	$sourceFilePath = $source->getFilePath();
            	$groupToProcessTogether[$sourceFilePath] = $sourceContent;
    ...
    protected function handleCssImports($css)
    {
        if ($this->options['bubbleCssImports']) {
            ...
            App::uses('UrlUtilComponent', 'Controller/Component');
            $urlUtil = new UrlUtilComponent(new ComponentCollection());
            $urlUtil->startup(new Controller());

            $loadImportedCssFileContent = true;
            $externalCssArr = [];
            $importedLocalCssArr = [];
            foreach ($imports as $importArr){
            	foreach($importArr as $import){
            		preg_match('/^@import\s(?:(url\())?(?:")?([a-zA-Z.-_0-9:\+\:\?,]+)(?:")?(?:\))?;$/i', trim($import), $matches);
            		if(empty($matches[2])){
            			$loadImportedCssFileContent = false;
            			break;
            		}else{
            			$cssFileUrl = $matches[2];
            			if(substr($cssFileUrl, 0, 2) == "//"){
            				$externalCssArr[] = $import;
            			}else{
            				$content = $urlUtil->fetchContent ( $cssFileUrl );
            				$importedLocalCssArr[$import] = $content;
            			}
            		}
            	}
            	if(!$loadImportedCssFileContent){ break; }
            }
            if($loadImportedCssFileContent && !empty($importedLocalCssArr)){
            	foreach ($importedLocalCssArr as $import => $localCssFileContent){
            		$css = str_replace($import, $localCssFileContent, $css);
            	}
            	if(!empty($externalCssArr)){
            		$css = implode('', $externalCssArr) . preg_replace('/@import.*?;/', '', $css);
            	}
            }else{
