 ...
 * List bucket from S3
 *
 * * Usage: $fileList = $this->{model}->amazonS3StorageManagement('LIST', '?prefix=tasks/6/&delimiter=/');
 *
 * @return void
 * @author Charles Li
 **/
	public function listBucket($remotePath, $returnDir = false) {
		...
		if (isset($headers['Content-Type']) && $headers['Content-Type'] == 'application/xml') {
			...
			$filePrefix = '';
			if(!empty($files->Prefix) && count($files->Prefix) > 0){
				$prefix = (array)$files->Prefix;
				$filePrefix = $prefix[0];
			}
			foreach ($files as $file){
				if(!empty($file->Key) && count($file->Key) > 0){
					$key = (array)$file->Key;
					$fileName = str_replace($filePrefix, "", $key[0]);
					$fileList[$fileName] = Configure::read('System.aws.s3.bucket.link.prefix') .$filePrefix .$fileName;
				}
			}

			if($returnDir && !empty($files->CommonPrefixes)){
				$filesArr = json_decode(json_encode($files), true);
				foreach ($filesArr['CommonPrefixes'] as $key => $prefix){
					if(!empty($filesArr['CommonPrefixes'][0])){
						$prefix = $prefix['Prefix'];
					}
					$k = str_replace($filesArr['Prefix'], "", $prefix);
					$k = preg_replace(array("/^\//", "/\/$/") , "", $k);
					$fileList[$k] = Configure::read('System.aws.s3.bucket.link.prefix'). $prefix;
				}
			}
		}
 ...
 **/
	public function checkRemoteDir($remoteDir) {
		...
		if ($remoteDir) {
			...
			$pathinfo = pathinfo($remoteDir);
			if(!empty($pathinfo['extension'])){
				$this->file = $remoteDir; // If dir already contains the file name, then use that file name
			}else{
