...
class AclController extends AclManagerAppController {
	...
	public function drop() {
		...
		$logMessage  = __('Drop all ACL permissions, please investigate this action. If this is OK, please ignore this report. (User: #' .$this->superUserId .')');
		$this->Log->addLogRecord($logType, $logLevel, $logMessage, true);

        $this->autoRender = false;
        $afterCloseFunc = 'function(){ window.location.href = "/admin/dashboard#/admin/acl_manager/acl/permissions/aro:Group"; }';
        $response = '<script>messageBox(' .json_encode(array('status' => Configure::read('System.variable.success'), 'message' => __('Both ACOs and AROs have been dropped'), 'sticky' => true, 'after_close' => "%function%")) .');</script>';
        echo str_replace("\"%function%\"", $afterCloseFunc, $response);
	}
	...
	public function drop_perms() {
		...
		if ($this->Acl->Aro->Permission->deleteAll(array("1 = 1"))) {
            $response = '<script>messageBox(' .json_encode(array('status' => Configure::read('System.variable.success'), 'message' => __('Permissions dropped'), 'sticky' => true, 'after_close' => "%function%")) .');</script>';
		} else {
            $response = '<script>messageBox(' .json_encode(array('status' => Configure::read('System.variable.error'), 'message' => __('Error while trying to drop permissions'), 'sticky' => true, 'after_close' => "%function%")) .');</script>';
		}
		echo str_replace("\"%function%\"", $afterCloseFunc, $response);
	}
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			foreach ($perms as $aco => $aros) {
				...
				foreach ($aros as $node => $perm) {
					...
					$acoId = $this->_getAcoIdFromTreePath($action);

					if($this->_checkPermissionExistence($acoId, $id, $action, $perm)){
						continue;
					}else{
						$actionReadable = str_replace("controllers/", "", $action);
					}

					if ($perm == 'allow') {

						$logType 	 = Configure::read('Config.type.user');
						if($this->Acl->allow($node, $action)){
							$logLevel 	 = Configure::read('System.log.level.info');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as ALLOW was done successfully.');
						}else{
							$logLevel 	 = Configure::read('System.log.level.warning');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as ALLOW was failed.');
						}
						...
						$logType 	 = Configure::read('Config.type.user');
						if($this->Acl->inherit($node, $action)){
							$logLevel 	 = Configure::read('System.log.level.info');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as INHERIT was done successfully.');
						}else{
							$logLevel 	 = Configure::read('System.log.level.warning');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as INHERIT was failed.');
						}
						$this->Log->addLogRecord($logType, $logLevel, $logMessage);
						...
						$logType 	 = Configure::read('Config.type.user');
						if($this->Acl->deny($node, $action)){
							$logLevel 	 = Configure::read('System.log.level.info');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as DENY was done successfully.');
						}else{
							$logLevel 	 = Configure::read('System.log.level.warning');
							$logMessage  = __('Update ' .$node['model'] .' (' .$this->Group->getGroupName($node['foreign_key']) .') permission (' .$actionReadable .') as DENY was failed.');
						}
						$this->Log->addLogRecord($logType, $logLevel, $logMessage);
					}
				}
            ...
            // Return after saving the permission
            $this->autoRender = false;
            echo json_encode(array('status' => Configure::read('System.variable.success'), 'message' => __("Permissions updated")));
            return;
		}
	...
	/**
	 * Travel the tree based on the path and get the last leaf id
	 * @param string|array $path
	 * @return id
	 */
	private function _getAcoIdFromTreePath($path, $parentId = null){
		if(!is_array($path)){
			$path = explode("/", $path);
		}
		$nodeAlias = array_shift($path);
		$node = $this->Acl->Aco->find('first', array(
			'fields' => array('id'),
			'conditions' => array('parent_id' => $parentId, 'alias' => $nodeAlias),
			'recursive' => -1
		));

		if(count($path) > 0){
			return $this->_getAcoIdFromTreePath($path, $node['Aco']['id']);
		}else{
			return $node['Aco']['id'];
		}
	}
	...
	/**
	 * Check whether the permission exists or not. If exists, ignore it.
	 * Because when update ACLs, we send the whole form here and actually we only want to change one ARO's permission
	 * @param int $aroId [Notice: this param is not $aroId, it is the foreign key of aro (model)]
	 * @param int $acoId
	 * @param string $perm
	 * @return boolean
	 */
	private function _checkPermissionExistence($acoId, $aroId, $action, $perm){

		$db = $this->Acl->Aco->getDataSource();
		$result = $db->fetchAll(
		    'SELECT _create, _read, _update, _delete FROM aros_acos WHERE aro_id = (SELECT id FROM aros WHERE foreign_key = ?) AND aco_id = ?',
		    array($aroId, $acoId)
		);

		$currentPermission = null;
		if($result[0]['aros_acos']['_create'] == 0 && $result[0]['aros_acos']['_read'] == 0 && $result[0]['aros_acos']['_update'] == 0 && $result[0]['aros_acos']['_delete'] == 0){
			$currentPermission = 'inherit';
		}elseif($result[0]['aros_acos']['_create'] == 1 && $result[0]['aros_acos']['_read'] == 1 && $result[0]['aros_acos']['_update'] == 1 && $result[0]['aros_acos']['_delete'] == 1){
			$currentPermission = 'allow';
		}elseif($result[0]['aros_acos']['_create'] == -1 && $result[0]['aros_acos']['_read'] == -1 && $result[0]['aros_acos']['_update'] == -1 && $result[0]['aros_acos']['_delete'] == -1){
			$currentPermission = 'deny';
		}

		return !empty($perm) && ($currentPermission === strtolower($perm));
	}
	...
	public function update_acos() {
		...
		$this->autoRender = false;
		$afterCloseFunc = 'function(){ window.location.href = "/admin/dashboard#/admin/acl_manager/acl/permissions/aro:Group"; }';
		$response = '<script>messageBox(' .json_encode(array('status' => Configure::read('System.variable.success'), 'message' => sprintf(__("%d ACOs have been created/updated"), $count), 'sticky' => true, 'after_close' => "%function%")) .');</script>';
        echo str_replace("\"%function%\"", $afterCloseFunc, $response);

        $logType 	 = Configure::read('Config.type.user');
		$logLevel 	 = Configure::read('System.log.level.info');
		$logMessage  = __('Update ACOs');
		$this->Log->addLogRecord($logType, $logLevel, $logMessage);
	}
	...
	public function update_aros() {
        ...
        $this->autoRender = false;
        $afterCloseFunc = 'function(){ window.location.href = "/admin/dashboard#/admin/acl_manager/acl/permissions/aro:Group"; }';
        $response = '<script>messageBox(' .json_encode(array('status' => Configure::read('System.variable.success'), 'message' => sprintf(__("%d AROs have been created/updated"), $count), 'sticky' => true, 'after_close' => "%function%")) .');</script>';
        echo str_replace("\"%function%\"", $afterCloseFunc, $response);

        $logType 	 = Configure::read('Config.type.user');
        $logLevel 	 = Configure::read('System.log.level.info');
        $logMessage  = __('Update AROs');
        $this->Log->addLogRecord($logType, $logLevel, $logMessage);
	}
