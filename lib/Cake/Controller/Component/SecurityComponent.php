 ...
 */
	protected function _validatePost(Controller $controller) {
		...
		if(Configure::read('debugSecurity')){
			error_log("Token to be validated in SecurityComponent");
			error_log($token);
		}
		...
		if(Configure::read('debugSecurity')){
			error_log("Fields from SecurityComponent: ");
			error_log(print_r($fieldList, true));
			error_log("Locked from SecurityComponent: ");
			error_log(print_r($lockedFields, true));
			error_log("Unlocked from SecurityComponent: ");
			error_log(print_r($unlocked, true));
		}

		$check = Security::hash(serialize($fieldList) . $unlocked . Configure::read('Security.salt'), 'sha1');

		if(Configure::read('debugSecurity')){
			error_log("Token tvalidation result in SecurityComponent");
			error_log($token .' === ' .$check);
		}
